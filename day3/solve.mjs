...
function bitFlip(byteString) {
    return byteString.split("").map(b => b === "0" ? "1" : "0").join("")
}
...
const o2rate = filterByFrequencyWindow(data, buildFrequencyMap, reduceToMostCommonBits)
const co2rate = filterByFrequencyWindow(data, buildFrequencyMap, reduceToLeastCommonBits)
